...
public class EmployeeRestController {
    ...
    public ResponseEntity<Employee> getEmployeeById(@PathVariable("id") String id) {
        Employee employee = employeeServiceImp.getEmployeeById(Integer.parseInt(id));
    ...
    public ResponseEntity<String> updateEmployeeById( @PathVariable("id") String id , @ModelAttribute Employee employee, consumes = MediaType.APPLICATION_JSON_VALUE) {
        ...
        try {
             ...
             if( result > 0 ) {
          	...
          	return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("failed to update employee! try out again!");
          }
        }
